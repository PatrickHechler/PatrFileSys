...
public class PatrFSProvider extends FSProvider {
	...
	private static final SymbolLookup GLIBC_LIBARY_LOCKUP = SymbolLookup.libraryLookup("/lib/libc.so.6",
	...
	private static void loadPatrRamOpts(Linker linker, PatrRamFSOpts opts) throws Throwable {
		MethodHandle newBm            = linker.downcallHandle(
				PatrFS.LOCKUP.lookup("bm_new_ram_block_manager").orElseThrow(),
				FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.JAVA_LONG, ValueLayout.JAVA_INT));
		MethodHandle pfsLoadAndFormat = linker.downcallHandle(PatrFS.LOCKUP.lookup("pfs_load_and_format").orElseThrow(),
	...
	private static void loadPatrOpts(Linker linker, MemorySession local, PatrFSOptions opts) throws Throwable {
		MethodHandle  open  = linker.downcallHandle(GLIBC_LIBARY_LOCKUP.lookup("open").orElseThrow(), FunctionDescriptor
				.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT));
		MethodHandle  newBm = linker.downcallHandle(PatrFS.LOCKUP.lookup("bm_new_file_block_manager").orElseThrow(),
	...
	private static void loadWithoutFormat(Linker linker, MemorySession local, PatrFSOptions opts, MethodHandle newBm,
			int fd) throws Throwable {
		MethodHandle  pfsLoad = linker.downcallHandle(PatrFS.LOCKUP.lookup("pfs_load").orElseThrow(),
		...
		if (0 == (int) pfsLoad.invoke(bm, MemoryAddress.NULL)) { throw thrw(PFSErrorCause.LOAD_PFS, opts.path()); }
	}
