...
/**
 * the {@link FSProvider} provides methods to load simple file systems with the interface {@link FS}
 * 
 * @author pat
 */
public abstract class FSProvider {
	 ...
	 * <p>
	 * <b>WARNING:</b> this provider is only ensured to work on LINUX.<br>
	 * make sure the binaries are
	 */
	public static final String PATR_FS_PROVIDER_NAME = "patr-fs";
	/**
	 * the {@link #name()} of the {@link FSProvider}, which delegates to a {@link Path}
	 */
	...
	/**
	 * creates a new file system provider with the given name and given maximum file system load count
	 * 
	...
	/**
	 * returns the maximum number of file systems which can be loaded at a time by this file system provider
	 * 
	 * @return the maximum number of file systems which can be loaded at a time by this file system provider
	 */
	public final int maxLoadedFSCount() {
		return this.maxFSCount;
	}
	
	/**
	 * loads a new file system with the given load options
	 * <p>
	 * the implementation may require the {@link FSOptions} {@code opts} to be a special class.<br>
	 * for example the {@value #PATR_FS_PROVIDER_NAME} requires the {@code opts} to be {@link PatrFSOptions}.
	 * 
	...
	/**
	 * returns a {@link Collection} which contains all file systems which has been loaded by this provider and where the {@link FS#close()} method has not yet been
	 * called
	 * <p>
	 * it is up to the implementation if the returned {@link Collection} is modifiable and if the returned {@link Collection} changes when {@link #loadedFS()} and
	 * {@link FS#close()} are called.
	 * 
	 * @return a {@link Collection} which contains all file systems which has been loaded by this provider and where the {@link FS#close()} method has not yet been
	 *             called
	 */
	...
	public String toString() {
		return "FSProvider [name=" + this.name + "]";
	}
	
	/**
	 * this map saves all installed {@link FSProvider file-system-providers} mapped with their {@link #name}
	 */
	 ...
	 * 
	 * @return a map containing all installed {@link FSProvider file-system-providers} mapped with their {@link #name}
	 */
	...
	/**
	 * returns a {@link Map} containing the file system providers mapped with the {@link #name()}
	 * 
	 * @return a {@link Map} containing the file system providers mapped with the {@link #name()}
	 */
