...
extern int pfs_mount_get_mount_point(int eh) {
	...
	if (handle->direct_parent_place.block == -1) {
		if (handle->is_mount_point) {
			// if this already is a mount point, return the mount point,
			// on which the mount point entry lies. (not the mount point itself)
			handle = &handle->fs_data->mount_point->handle;
		} else {
			// if this is the root folder, fail
			pfs_err = PFS_ERRNO_ROOT_FOLDER;
			return -1;
		}
	}
	return_handle(pfs_eh_len, pfs_ehs, handle->fs_data->root);
}
...
extern enum mount_type pfs_mount_fs_type(int eh) {
	eh(-1)
	check_mount_point(-1)
	struct pfs_element_handle_mount *mp =
			pfs_ehs[eh]->handle.fs_data->mount_point;
	if (mp) {
